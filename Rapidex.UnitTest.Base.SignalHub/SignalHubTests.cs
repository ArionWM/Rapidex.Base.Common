...
public class SignalHubTests : IClassFixture<SingletonFixtureFactory<DefaultEmptyFixture>>
{
    ...
    public async Task T01_Basics_SubscribeAndFind()
    {
        ...
        IResult<int> subs1Result = await messageHub.Subscribe(null, "/+/base/+/new/myEntity", (args) =>
           {
               return args;
           });
        Assert.True(subs1Result.Success);

        IResult<int> subs2Result = await messageHub.Subscribe(null, "/myTenant1/base/+/new/myEntity", (args) =>
        {
            return args;
        });
        ...
        Assert.Equal("myTenant1", parseResult1.Topic.Tenant);
        ...
        Assert.Equal("myTenant2", parseResult2.Topic.Tenant);
    ...
    public async Task T02_Basics_Invoke()
    {
        ...
        await messageHub.Publish("/myTenant2/base/+/new/myEntity", args01);
        ...
        await messageHub.Publish("/myTenant1/base/+/new/myEntity", args02);
        Assert.Equal(1, subs1Invokes.Count);
